...
class GameContainer extends React.Component {
  ...
  findIndex = (currentCard) => {
    let index = this.state.shuffledCards.findIndex(
  ...
  reviewGame = () => {
    return this.state.completedCards.map((card) => (
      <FlashCard
        key={card.id}
        card={card}
        wonStatus={card.right}
      ...
      <section className="game-container">
        <section className="game-container-left-section">
          ...
          <div className="game-stats">
            ...
            this.state.shuffledCards.length ? (
              <h1 className="percent-right-num">
                {Math.round(
                  (this.state.rightCounter / this.state.shuffledCards.length) *
                    100
                )}
                %
              </h1>
          ...
          {this.state.currentCardIndex + 1 > this.state.shuffledCards.length ? (
            ...
            <div className="game-container-right-section-session">
              ...
              <div className="game-buttons">
                <Button id="right-button" onClick={this.handleRight}>
                  Right
                </Button>
                <Button id="wrong-button" onClick={this.handleWrong}>
                  Wrong
                </Button>
                <Button id="skip-button" onClick={this.handleSkipped}>
                  Skip
                </Button>
              </div>
            </div>
