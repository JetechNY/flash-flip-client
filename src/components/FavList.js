...
class FavList extends React.Component {
  ...
  renderCards = () => {
    return this.state.favCards.map((card) => (
      <FlashCard
        handleDeleteCard={this.handleDeleteCardInFavList}
        jwt={this.props.jwt}
        key={card.id}
        card={card}
        parentIsFavList={true}
      />
    ));
  };
  ...
  render() {
    return (
      <div className="fav-list">
        ...
        <div className="fav-list-container">{this.renderCards()}</div>
      </div>
    );
  }
}
